...
async function worker() {
  ...
  for (;;) {
    ...
    try {
      ...
      if (new URL(request.url).hostname === 'guide.michelin.com' && request.url.includes('/restaurant/')) {
      ...
      } else if (new URL(request.url).hostname === 'guide.michelin.com') {
        await page.waitForFunction(() => document.body.innerText.length > 2000, { timeout: 25000 });
      } else if (new URL(request.url).hostname.endsWith('bing.com')) {

...
for (const city of catalog.cities) {
  for (const guide of city.guides) {
    ...
    datasets.push({
      dataset: `${city.id}/${guide.year}/${guide.id}`, listed: after.length,
      changed_in_this_stage: city.id === 'tokyo',
      coverage_status: guide.coverage.status,
...
writeFileSync(new URL('./distribution.json', import.meta.url), JSON.stringify({
  method: 'Production diningDistribution; largest icon groups include unclassified in other; denominator = listed.',
  datasets,
}, null, 2) + '\n');
for (const dataset of datasets.filter(row => row.changed_in_this_stage)) console.log(JSON.stringify(dataset));

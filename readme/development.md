...
| 线上打不开、提示断网，或仍显示旧版 | 按[线上访问与版本排障](#线上访问与版本排障)先核对网络响应和浏览器实际构建；单凭此现象不能判定部署失败。 |
...
本节是维护者和后续 Agent 的发布操作说明。接手目标是把指定候选交付到正确站点，准确说明验证范围，并能定位访问异常、恢复已验证产物。修改发布行为时同步本节；README 和 runbook 索引只提供入口，OpenSpec 保留实现与独立验收的历史，不另维护一份发布流程。

接手先确认下列事实，再选择当前可用的工具。命令示例不是唯一方法，也不要求安装某个 CLI 或使用特定浏览器。

| 要确认的事实 | 权威来源与判断边界 |
|---|---|
| 要交付哪个候选、哪些动作已获授权 | 当前用户任务、Git 工作区和提交；确认包含的改动，保留已有未提交工作。查阅文档或 CI 通过本身不授权发布；已有明确授权时继续执行，不重复索要同一确认。 |
| 代码是否已经到 GitHub | 当前 remote 配置和 GitHub 实际分支 SHA；本地跟踪分支可能过期，本机镜像也不代表 GitHub。 |
| 这次运行会检查还是发布 | 候选提交中的工作流及实际运行的 event、ref、head SHA、job 状态；不能只看工作流名称或总状态。 |
| 哪个地址、哪份内容已交付 | CNAME、Pages 设置、对应运行的发布包/凭据和线上核验；本地重新构建不能代替原发布包。 |
| 用户实际收到什么 | 完整 URL、网络环境、浏览器/PWA 状态、页面构建身份与具体操作结果；单个网络上的结果只证明该次访问。 |
...
没有 `gh` 时可使用 Git 和 GitHub 网页完成已有权限允许的工作；已授权且符合仓库规则的快进合并同样可以交付已检查提交。日志/API 的 403 需区分限流与缺少认证，不能当作 CI 或站点失败，也不要无效重复请求。网页摘要能证明可见 job 的状态，不能冒充已下载、逐项读过的日志或 artifact；取证工具不可用时说明缺少哪项证据，继续可独立完成的检查。
...
报告结果时分别说明下面三件事，结论与实际证据对应：

| 结论 | 足够的证据 | 不由此推定 |
|---|---|---|
| 候选检查通过 | 指定提交的 `Release quality` 完成且成功，必需门禁未跳过 | 已部署到 Pages；所有外部网络和真机可用 |
| 发布及线上文件核验通过 | 对应 `main` 运行的 `Deploy GitHub Pages` 成功，实际执行了部署和 `Verify public HTTPS and deployed files`；保存该运行 URL 与原始发布包 | 每个用户网络都可达；已有浏览器缓存已升级；实际页面交互已验证 |
| 用户访问/功能确认通过 | 在注明网络、浏览器和构建的正式网址完成本次变更相关操作，或明确标注为用户反馈 | 所有地区、浏览器、设备和未执行场景都通过 |

`release:smoke` 使用 Node HTTP 请求比较线上字节，不执行页面 JavaScript、Service Worker 或地图瓦片。`test:e2e` 的浏览器场景运行于本地生产构建及受控网络；即使内容相同，也不能写成“已经在正式域名浏览器里点选东京”。PR、`ci/**`、定时或 `verify` 运行可以整体成功但部署 job 被跳过。

正式域名可达时，选择与本次变更有关的用户操作抽检，例如打开城市菜单、切换新城市/年度、搜索并查看餐厅详情。涉及升级时保留原浏览器状态，并与新浏览器上下文比较。无法访问时仍可如实报告“GitHub 侧发布及文件核验通过，当前网络的浏览器抽检未完成”；本机受阻不推翻已有发布证据，远端成功也不能抹去用户的访问故障。

在本次 PR、任务记录或给用户的交付说明中，至少保留：提交 SHA 和运行 URL、已完成的检查/部署/线上核验、访问验证的网络与浏览器及结果、原发布包/保留期，以及尚未证实的现象或确实需要用户完成的一步。无需另建发布登记表或强制模板。原始证据归该运行 artifacts 或 `test-results/`；历史示例与旧测试次数不能代替下一次验证。

### 线上访问与版本排障

先保留现状，收集完整地址、具体表现、发生时间及网络/浏览器背景。检查方式由证据决定；不要因一个“无法访问”就改代码、重发版本、降低门禁或清空缓存。

| 观察到的现象 | 下一步与结论边界 |
|---|---|
| DNS 失败、连接重置、超时、`No internet connection` | 核对 DNS 与 TLS/HTTP 实际结果。TLS 握手失败时页面应用尚未加载，不能据此认定城市或数据丢失；错误文案本身不足以定位原因。 |
| HTTP 403/503 | 查看响应头和响应体，区分 GitHub、公司网关、代理、Service Worker 等来源。公司策略拦截页属于该访问路径的问题；分类标签不能单独证明网站有恶意内容。不要只看到状态码就归为部署失败。 |
| GitHub runner 可达，本机或用户不可达 | 对同一 URL 比较网络路径。记录公司网络、个人网络及用户已在使用的 VPN/代理等差异；若需改设置或处理公司策略，由用户/管理员决定。VPN 可达是一次环境对照，不是本项目永久要求，也不是 Agent 应自动切换网络的指令。 |
| 能打开旧页面，但新城市链接打不开 | 可能存在旧缓存与网络不通叠加；先比对实际构建，再判断。保留原浏览器/PWA，使用独立的新上下文取证；不要一开始清缓存而丢失现象。 |
| 页面正常但城市列表缺项 | 当前 city 参数只选择城市，不隐藏其他可选城市。核对年度与当前 catalog，再实际打开顶部城市菜单并滚动完整列表。若服务器内容、新浏览器与现有页面不同，分别记录身份；不能仅凭“本地有该数据”断言用户页面一定有。 |
| 控件和餐厅正常、只有地图空白 | 独立检查外部瓦片供应商请求；应用数据可用与地图底图可用不是同一个结果。 |

原始 `verified.json` 的 `head`、`source.sha256`、`artifact.sha256` 对应源码提交、检查输入和完整产物；`release.json` 的 `buildId`/`sourceSha256` 对应构建身份。页面 `.dataset-status` 的 `data-build`/`data-revision` 表示正在使用的构建和数据，状态文字的 title 也显示简短标识。`VERSION` 是展示版本，不是唯一发布身份，历史上不同构建都显示过 `v0.3.0`。发布前检查展示版本是否需更新，版本值归 VERSION，不能靠改版本号诊断网络或缓存。

服务器与浏览器要分别取证：原浏览器访问 `/release.json` 也可能经过活动 Worker，查询参数或普通刷新并不保证绕过它。独立 HTTP 客户端可检查服务器交付，新浏览器上下文可检查首次访问，原页面可检查已有用户实际状态；注明各自网络和构建后再比较。新 Worker 需完整下载并校验壳资源，旧页面全部关闭后才接管；已有 PWA 窗口也算页面。网络恢复前反复刷新或清空好缓存不保证解决问题。

只有证据指向发布包、部署步骤或应用行为回归，才进入相应修复或[恢复历史发布](#恢复历史发布)。已确认网络路径受阻时，保留“发布成功”和“该网络访问失败”两个事实，处理网络访问条件，不用重新部署掩盖诊断缺口。

### 已确认的交接实例（2026-09-16）

提交 `beff007ca4859123c4d0ba3fbcef1cb5bbb318ae` 的[正式发布运行](https://github.com/yanrucheng/foodie-map/actions/runs/35057103180)完成质量门禁、Pages 部署、HTTPS 及全部线上文件核验，并保存 `release-1` 与 `deployment-evidence-1`。这是该次运行的历史事实，不是永久的“当前线上版本”。

同次排查中，Agent 所在网络的 HTTP 响应是公司安全策略拦截页，HTTPS 在握手阶段重置；用户随后确认启用其 VPN 后可以访问，认可本次发布成功。浏览器之前未见东京是否来自旧缓存没有被直接证实，不写成确定根因。这次经验是区分发布证据、访问路径和浏览器状态；不把 VPN、旧版本号或特定工具作为后续发布的固定前提。
...
P03 年度迁移与 P04/P05 正式发现联调已完成，旧 JSON 地址固定派生自 catalog 指定的年度，校验会拒绝不一致的副本。BC-01 退役政策仍待决，不承诺永久兼容。当前内容的 11 份历史名单为 unverified，东京两份为 partial；这是如实记录的资料边界，不因门禁成功改成名单完整。P05 新增按 catalog 选择底图，东京已从高德切到 GSI 标准图；历史在线取证已清理，当前覆盖和精度需按数据接入指南重新抽检。大陆/香港/澳门独立数值与物理精度仍缺证据，东京地图参考也不等于全部餐厅或实地 GPS 精度通过。远端 CI 和正式发布已有上述具体运行证据；P06 剩余人工范围及独立 G1–G4 判定仍须分别完成，不能由发布成功代签。

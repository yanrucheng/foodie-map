...
export function restaurantFacts(record: Restaurant, groupLabel = record.cuisine_group, spatialContext?: SpatialContext) {
  ...
  const details: [string, string | null | undefined][] = [
    ...
    ["价格等级", priceGrade.status === "valid" ? `${priceGrade.badge}（价格等级 ${priceGrade.tier}，共 4 档；原文 ${record.price_range}）` : hasText(record.price_range) ? `${record.price_range}（未识别等级）` : null],
  ...
  return {
    ...
    tags: [groupLabel, category.label, record.cuisine, `${record.edition_year} ${guide}`, record.star_rating ? `${record.star_rating} 星` : null, record.is_new === true ? `${record.edition_year} 新晋` : null].filter(hasText),

...
for (const engine of ["chromium", "webkit"]) for (const width of [1280, 390]) {
  test(`P09 R1–R7 ${engine} ${width}px: raw catalog, forms, dense targets, context stability and offline icons`, async () => {
    ...
    try {
      ...
      assert.deepEqual((await page.locator(".dining-segment-btn").allTextContents()).map((text) => text.trim()), ["全部 12", "面饭面点 1", "肉食主打 2", "鱼鲜主打 1", "甜饮 2", "法式 1", "中餐 1", "日式会席 1", "其他料理 3"]);
      assert.doesNotMatch((await page.locator('.control-block').allTextContents()).join('\n'), /未标注/);
      assert.match(await page.locator(".dining-count-note").first().textContent(), /全部收录/);
      ...
      assert.match(await page.locator('[data-dining="meat"]').textContent(), /肉食主打 2/);
      await page.locator('[data-dining="other"]').click(); await count(page, 3, 2);
      ...
      await filterPanel(page, mobile);
      assert.equal((await page.locator('[data-dining="other"]').textContent()).trim(), "其他料理 12");
      assert.doesNotMatch((await page.locator('.control-block').allTextContents()).join('\n'), /未标注/);

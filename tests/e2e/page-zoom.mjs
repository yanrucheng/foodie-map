...
import { fixture, initialUrl } from "./accessibility.helpers.mjs";
...
await build({ logLevel: "error", define: { __FOODIE_RELEASE__: "null" }, plugins: [{ name: "p06-zoom-fixture", enforce: "pre", load(id) {
...
try {
  ...
  for (const viewport of [{ width: 1280, height: 800 }, { width: 768, height: 1024 }]) {
    ...
    const prefix = `p06-native-200-${viewport.width}`;
  ...
  await writeFile(join(artifacts, "p06-native-zoom.json"), JSON.stringify({ browser: await browser.version(), mechanism: "Chrome extension tabs.setZoom(tabId, 2); tabs.getZoom confirms 2; native window resized via Browser.setWindowBounds", fixture: "P04 guide-experience + P06 in-memory multi-guide fixture", evidence }, null, 2));

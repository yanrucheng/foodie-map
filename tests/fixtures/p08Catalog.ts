...
/** P09 extends the existing visual fixture: isolated semantics and dense coordinates; no claim about real restaurants. */
  ...
  for (const [cityIndex, city] of catalog.cities.entries()) {
    ...
    for (const guide of city.guides) {
      const overrides: Partial<Restaurant>[] = [
        { name: "餐食测试", serving_form: "meal", dining_category: "meat", price_range: "¥¥¥¥", venue_type: "street_food", is_new: true },
        { name: "小食测试", serving_form: "snack", dining_category: "staple", price_range: " $$ ", venue_type: "restaurant" },
        { name: "甜品测试", serving_form: "dessert", dining_category: "dessert_drink", price_range: "￥￥￥" },
        { name: "饮品测试", serving_form: "drink", dining_category: "dessert_drink", price: "150–250", currency: "JPY", lat: 35.68001 },
        { name: "未标注测试", venue_type: "dessert", price_range: '<img src=x onerror="window.p09Injected=true">' },
        { name: "空值无坐标", serving_form: null, dining_category: null, lat: null, lon: null },
        { name: "第二品类餐食", cuisine: "第二品类原文", cuisine_group: "SECOND_CATEGORY", serving_form: "meal", dining_category: "meat", price_range: "¥" },
        { name: "鱼鲜测试", dining_category: "seafood", price_range: "€€" },
        { name: "法式测试", dining_category: "french", price_range: "£" },
        { name: "中餐测试", dining_category: "chinese", price_range: "¥¥" },
        { name: "会席测试", dining_category: "japanese_course", price_range: "¥¥¥" },
        { name: "其他料理测试", dining_category: "other", price_range: "¥¥" },
      ];
      payloads[guide.dataPath!] = overrides.map((fields, index): Restaurant => ({
        ...
        ...(guide.id === "michelin-starred" ? { price_range: "¥¥¥¥" } : {}),
        ...(cityIndex && guide.year === 2027 && guide.id === "michelin-starred" ? { dining_category: null } : {}),
        // A separate city keeps individually visible targets during search flights.
        ...(cityIndex && fields.lat !== null ? {
          lat: [35.68, 35.68, 35.683, 35.677, 35.683, 35.68, 35.677, 35.686, 35.674, 35.68, 35.686, 35.674][index],
          lon: [139.757, 139.763, 139.757, 139.757, 139.763, 139.76, 139.763, 139.76, 139.76, 139.766, 139.766, 139.766][index],
        } : {}),
      }));
    }
  }
